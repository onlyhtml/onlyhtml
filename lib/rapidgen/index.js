...
export default class RapidContentFetcher {
    ...
    async fetchSpecificRecords(blockName, recordId) {
        ...
        values[blockName] = values[blockName][recordId];
        return values;
    }
    ...
    generateContent(blocks) {
        // console.log('blocks', Array.from(blocks).map(b => b.fields));
    ...
    generateFieldContent(field) {
        ...
        switch (type) {
            ...
            case 'reference': // TODO implement this correctly
            case 'text':
                if (params.words)
            ...
            case 'reference':
                // TODO implement this
                console.log('ref to', params.target);
                ...
                if (params.multiple === true) {
                    return (new Array(params.lorem_count || 1).fill()).map(_ => genWord());
                }
