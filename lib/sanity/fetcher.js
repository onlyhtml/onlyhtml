...
export default class SanityFetcher {
    ...
    subscribeToChanges(callback) {
        ...
        this._subscription = this.sanity.listen(this.query, this.params).subscribe((update) => {
    ...
    async fetchRecords() {
        ...
        for (const doc of sanityDocs) {
            if (this.blocks[doc._type].type == BlockType.COLLECTION) {
                ...
                records[doc._type].push(this._prepareDoc(doc));
                continue;
            }

            records[doc._type] = this._prepareDoc(doc);
        }
