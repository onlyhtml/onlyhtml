    ...
    async _prepareDirectChildren(doc) {
        ...
        for (const childBlock of block.directChildren) {
            ...
            if (childBlock.type === BlockType.COLLECTION) {
                let internalArray = doc[childBlock.id];
                internalRecords[childBlock.id] = await Promise.all(internalArray.map(doc => { 
            ...
            internalRecords[childBlock.id] = await this._prepareDocWithBlock(doc[childBlock.id], childBlock);
        }
