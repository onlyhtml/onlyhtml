...
describe('rendering vapid syntax templates', function () {
    ...
    describe("block rendering", function () {
        ...
        it("should render collection block with ic field", function() {
            const template = '{{#collection my_collection}}{{ic field}},{{/collection}}'
            const params = {
                my_collection: [{field: 'a'}, {field: 'b'}, {field: 'c'}]
            }
        ...
        it("should render fields with corrent parents even within partials", function() {
            ...
            template.registerPartialString('my_partial', `{{ic field type="text"}}`);
    ...
    describe("ic directives", function () {
        // it("render ic image component", function () {
        //     const url = '11-2222-333';
        //     const template = '{{ic image_field type="image"}}';
        ...
        //     const template = `{{ic image_1 type="image" width=20}}{{ic image_1 type="image" width=32}}`;
        ...
        it("dont allow ic block", function () {
            const template = '{{#ic image_1 type="image"}}{{/ic}}';
            const values = {general: {image_1: 'IMAGE_URL'}};
            assert.throws(() => {render(template, values)}, '');
        });

        it('parse a complex ic directive', function() {
            const template = new Template();
            const blocks = template.parse('{{ic some_icon type="editor"}}');
...
describe('parse templates', function () {
    ...
    it("parse one complex mustache field", function () {
        const structure = parse('{{ic aa type="image" class="w-full"}}');
        assert.deepEqual(
            structure,
    ...
    it("parse block with field", function () {
        const structure = parse('{{#page home_page}} {{ic aa}} {{/page}}');
        assert.deepEqual(
            structure,
            [
                new Block('general'),
    ...
    it("parse block numeric name", function () {
        const structure = parse('{{#page 404}} {{ic aa}} {{/page}}');
        assert.deepEqual(
            structure,
            [
                new Block('general'),
    ...
    it("parse page with inner section and field", function () {
        const structure = parse('{{#page home_page}} {{#section contact}} {{ic aa}} {{/section}} {{/page}}');
    ...
    it("parse page with hash parameter", function () {
        const structure = parse('{{#page home_page slug="/111"}} {{ic aa}} {{/page}}');
        assert.deepEqual(
            structure,
            [
                new Block('general'),
    ...
    it("parse conditional field", function () {
        const structure = parse('{{#choice is_rtl}} {{ic aa}} {{/choice}}');
        assert.deepEqual(
            structure,
    ...
    it("parse conditional field and hash parameter", function () {
        const structure = parse('{{#choice is_rtl type="dropdown"}} {{ic aa}} {{/choice}}');
        assert.deepEqual(
            structure,
    ...
    it("parse contents of if block", function() {
        const structure = parse('{{#if is_rtl}} {{ic aaa}} {{/if}}');
        assert.deepEqual(structure, [
    ...
    it("parse contents with partial", function() {
        const template = new Template();
        template.registerPartialString('my_partial', '{{ic aaa}}');
    ...
    it("parse contents with partial slash name", function() {
        const template = new Template();
        template.registerPartialString('my_partial/aa', '{{ic aaa}}');
    ...
    it("parse contents with partial with section", function() {
        const template = new Template();
        template.registerPartialString('my_partial', '{{#page home_page}} {{ic aaa}} {{/page}}');
        const structure = template.parse('{{> my_partial}}');
    ...
    it("should be able to parse two blocks with same id", function() {
        const template = new Template();
        const structure = template.parse('{{#section section_key}} {{ic field_a}} {{/section}} {{#section section_key}} {{ic field_b}} {{/section}}');
        assert.deepEqual(structure, [
            new Block('general'),
    ...
    it("should be able to parse three blocks with same id", function() {
        const template = new Template();
        const structure = template.parse('{{#section section_key}} {{ic field_c}} {{/section}}{{#section section_key}} {{ic field_a}} {{/section}} {{#section section_key}} {{ic field_b}} {{/section}}');
        assert.deepEqual(structure, [
            new Block('general'),
            new Block('section_key', BlockType.SECTION, {}, [
                NewField('field_c'),
    ...
    it("dont allow strange block inheritence", function() {
        const template = new Template();
        const cases = [
            '{{#section section_key}}{{ic field_a}}{{/section}}{{#page section_key}}{{aa}}{{/page}}', // same section id with different types
            '{{#page page_id}} {{ic field_a}} {{{#page page_id_2}}{{aa}}{{/page}} {/page}}', // page inside a page
            '{{#section block_id}} {{ic field_a}} {{{#page page_id_2}}{{aa}}{{/page}} {/section}}', // page inside a section 
            '{{#repeat block_id}} {{ic field_a}} {{field_b}} {{{#page page_id_2}}{{aa}}{{/page}} {/repeat}}', // page inside a repeat 
        ];
    ...
    describe("block inheritence", function() {
        ...
        it("page with primary child", function() {
            const template = new Template();
            const structure = template.parse("{{#page main_page}} {{#section child}} {{ic field}} {{/section}} {{/page}}");
        ...
        it("page with repeat child", function() {
            const template = new Template();
            const structure = template.parse("{{#page main_page}} {{#collection child}} {{ic field}} {{/collection}} {{/page}}");
            const mainPage = findBlock(structure, 'main_page');
        ...
        it("block inheritence should work through paritals", function() {
            const template = new Template();
            template.registerPartialString('my_partial', '{{#section child}} {{ic field}} {{/section}}');
    ...
    describe("local mustache field", function() {
        ...
        it("rendering of `local` fields should work", function() {
            ...
            const rendered = template.render("{{#section general}} {{ic field_a}}|{{local field_b}} {{/section}}", params).trim();
