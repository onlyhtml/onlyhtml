...
describe('parse templates', function () {
    ...
    it("parse page with inner section and field", function () {
        ...
        assert.deepInclude(structure[1].getDirectChildren(), contact);
    });
    ...
    describe("block inheritence", function() {
        ...
        it("collection item with inner collection", function(){
            const template = new Template();
            const structure = template.parse(`
                {{#collection_item post}}
                    {{#collection labels}}
            ...
            const post = findBlock(structure, 'post');
            post.hasCollectionItems = false;

            const expectedBlock = new Block('post', 'collection', {}, []);
            expectedBlock.directChildren.push(new Block('labels', 'collection', {}, [
                NewField('label')
            ]));

            assert.deepEqual(post, expectedBlock, 'post != expectedBlock')
        });
