...
const NativeHandlebarsBlocks = ['each', 'if', 'with', 'unless', 'compare', 'contains'];
...
export class HBTemplateParser extends Visitor {
    constructor() {
        ...
        this.hasCollectionItems = {}; // blockId to boolean
    }
    ...
    getBlocks() {
        ...
        for (const [childId, parentIds] of Object.entries(this.blocksToParents)) {
            ...
            if (parentIds.length > 1) {
                ...
                // remove blocks if they are direct children as they are included under their parents
                directChildrenIDs.push(child.id);
            }
        }

        return this.blocks.filter(b => !directChildrenIDs.includes(b.id));
    }
    ...
    mustachestatement(mustacheNode) {
        if (mustacheNode.params) {
            // nodes might have subexpressions in their params
            for (const param of mustacheNode.params) {
                if (param.type === 'SubExpression') {
                    this.subexpression(param);
                }
            }
        }
    ...
    subexpression(subexpressionNode) {
        // console.log('found a sub expression');
        this.mustachestatement(subexpressionNode);
    }
