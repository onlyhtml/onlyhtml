...
export class CloudProvider {
    ...
    async _buildResources() {
        ...
        const resources = {}
        glob.sync(`${tmpDist}/**/*.{js,css}`).map(tmpFilePath=> {
            const filePath = tmpFilePath.replace(tmpDist+'/', '');
            resources[filePath] = fs.readFileSync(tmpFilePath, 'utf8');
        });
